...
func setupLevelYSort():
	var niveau: CanvasItem = $ZoneLevel/MarkerLevel.get_child(0)
	niveau.z_index = 0
	niveau.y_sort_enabled = true
	niveau.z_as_relative = false
	
	var obstacles = $ZoneLevel/MarkerLevel.get_child(0).get_node("Obstacles")
	obstacles.z_index = 1
...
func addplant(idxplant, newplant):
	var niveau = $ZoneLevel/MarkerLevel.get_child(0)
	...
	$ZoneLevel/MarkerLevel.get_child(0).add_child(newplant)
...
func addlevelmap(level: int):
	...
	var levelmap : Niveau = leveltscn.instantiate()
...
func spawnpersos():
	...
	$ZoneLevel/MarkerLevel.get_child(0).add_child(perso1)
	...
	$ZoneLevel/MarkerLevel.get_child(0).add_child(perso2)
